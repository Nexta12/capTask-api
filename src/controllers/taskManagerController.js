...
export const getAllTasks = asyncHandler(async (req, res) => {
  const { department, employeeName, startDate, endDate } = req.query;
  ...
  // Send success response
  responseHandler(res, 200, tasks, "Tasks retrieved successfully.");
});
...
export const exportSingleTaskToExcel = asyncHandler(async (req, res) => {
  ...
  worksheet.addRow({ field: "Created At", value: task.createdAt.toLocaleString() });
  worksheet.addRow({
    field: "Employee",
    value: `${task.employee.firstName} ${task.employee.lastName}`
  });
  worksheet.addRow({
    field: `${task.status} By`,
    value: `${task.remarkBy.firstName} ${task.remarkBy.lastName}`
  });
...
export const exportAllTasksToExcel = asyncHandler(async (req, res) => {
  // Fetch all tasks sorted by createdAt in descending order
